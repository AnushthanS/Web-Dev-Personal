...
const Navbar = () => {
  ...
  return (
    <nav
      className={`${styles.paddingX} w-full flex items-center py-5 fixed z-20 bg-primary top-0`}
    >
      <div className='w-full flex justify-between items-center max-w-7xl mx-auto'>
        <Link
          ...
          <span className='text-white text-[18px] font-bold cursor-pointer'>Anushthan Saxena</span>
        </Link>
            ...
            <ul className='list-none flex justify-end flex-col items-start gap-4'>
              {navLinks.map((link) => (
                <li
                  key={link.id}
                  className={`${active === link.title ? "text-white" : "text-secondary"
                    } hover:text-white text-[16x] cursor-pointer font-poppins font-medium text`}
                  onClick={() => { 
